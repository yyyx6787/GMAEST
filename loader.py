...
class BioDataset1(InMemoryDataset):
		def __init__(self,
					 root,
					 data_type,
					 empty=False,
			...
			super(BioDataset1, self).__init__(root, transform, pre_transform, pre_filter)
			if not empty:
		...
		def processed_file_names(self):
			return 'dataset_new_2.pt' #'NYC'
			# return "dataset_new_attr.pt" #"Chicago"
			# return 'sdata.pt' #'geometric_data_processed.pt'
# 
		# def download(self):
		...
		def process(self):
			raise NotImplementedError('Data is assumed to be processed')
class BioDataset_graphcl1(BioDataset1):
	def __init__(self,
				 root,
				 data_type,
				 empty=False,
		...
		super(BioDataset_graphcl1, self).__init__(root, data_type, empty, transform, pre_transform, pre_filter)
	...
	def node_drop(self, data):
		...
		data.edge_type = data.edge_type[idx_nondrop]
	...
	def edge_pert(self, data):
		...
		edge_attr_add = torch.tensor( np.random.randint(2, size=(edge_index_add.shape[1], 2)), dtype=torch.float32 )
		...
		# print("edge_attr_add:", edge_attr_add.size())
	...
	def subgraph(self, data):
		...
		data.edge_type = data.edge_type[edge_index]
		data.__num_nodes__, _ = data.x.shape
		return data
	...
	def generator_generate(self, data, n_generator=1):
		with torch.no_grad():
			prob, edge_attr_pred, edge_type_pred = self.generators[n_generator-1].generate(data)
		...
		for _ in range(10): # limit the walk within 10 steps
			...
			for n in idx_neigh:
				...
				# print("idx_neigh_n:", idx_neigh_n)
				_idx_neigh += [idx_neigh_n[0]]
				# print("**:", [(n, idx_neigh_n[0]), (idx_neigh_n[0], n)])
				# idx_edge += [(n, idx_neigh_n[0],n, idx_neigh_n[1]), (idx_neigh_n[0], n,idx_neigh_n[1], n)]
		...
		# print("idx_edge:", idx_edge)
		# println()
		idx_edge = torch.tensor(idx_edge).t().long()
		# print("idx_edge:",idx_edge)
		# println()
		# print("idx_edge[0]:",idx_edge[0], idx_edge[0].dtype)
		# print("idx_edge[1]:",idx_edge[1], idx_edge[1].dtype)
		# print("idx_edge[0]:",idx_edge[0])
		# print("idx_edge[1]:", idx_edge[1])
		# print("edge_attr_pred:", edge_attr_pred.size())
		edge_attr = edge_attr_pred[idx_edge[0].long(), idx_edge[1].long()]
		edge_type = edge_type_pred[idx_edge[0].long(), idx_edge[1].long()]
		# print("original edge type:", edge_type.size())
		...
		# edge_index_old, _ = tg_utils.subgraph(idx_sub, idx_edge, edge_attr=edge_attr, relabel_nodes=False, num_nodes=node_num)
		...
		# print("edge_attr:", edge_attr[:, :4].size())
		...
		data.edge_index = edge_index
		data.edge_attr = edge_attr[:, :4]
		data.edge_type = edge_attr[:, 4].view(-1,1)
		data.__num_nodes__, _ = data.x.shape
		return data
	...
	def get(self, idx):
		...
		for key in self.data.keys:
			...
			# print("##:",data.__cat_dim__(key, item), s)
			# print("^^:", slice(slices[idx], slices[idx + 1]))
			# print("s:", s)
			# print("item:", item[s])
			...
			# println()
			# data[key], data1[key], data2[key] = item, item, item
